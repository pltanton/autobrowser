...
with lib; let
  ...
  configText = ''
    ...
    ${lib.concatStringsSep "\n\n" (lib.mapAttrsToList (name: command: ''
        [command.${name}]
        cmd = ${builtins.toJSON (
          if builtins.isList command.cmd
          then command.cmd
          else command.cmd
        )}
    ...
    ${lib.concatStringsSep "\n\n" (map (rule: ''
        ...
        ${lib.concatStringsSep "\n" (map (matcher: ''
            ...
            ${lib.concatStringsSep "\n" (lib.mapAttrsToList (
              k: v:
                if k != "type" && v != null
                then "${(lib.replaceStrings ["displayName" "bundleId" "bundlePath" "executablePath"] ["display_name" "bundle_id" "bundle_path" "executable_path"] k)} = ${builtins.toJSON v}"
                else ""
